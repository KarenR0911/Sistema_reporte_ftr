...
export const ESTADOS_OPCIONES = Object.keys(ESTADOS_VENEZUELA).map((e) => ({
  value: e,
  label: e,
}))

export function municipiosDeEstado(estado: string): { value: string; label: string }[] {
  return (ESTADOS_VENEZUELA[estado] ?? []).map((m) => ({ value: m, label: m }))
}
